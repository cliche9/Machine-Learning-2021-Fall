    ...
    def reassign(self, img):
        ...
        for i in range(img.shape[0]):
            for j in range(img.shape[1]):
                k = np.argmin(linalg.norm(img[i][j] - self.mu, axis=1, keepdims=True) ** 2)
                img[i][j] = self.mu[k]

if __name__ == "__main__":
    large_img = cv2.imread('data6/bird_large.tiff')
    small_img = cv2.imread('data6/bird_small.tiff')
    kmeans = Kmeans(small_img)
    kmeans.train()
    kmeans.reassign(large_img)
    cv2.imwrite('data6/bird_large_after_all.tiff', np.uint8(np.round(large_img)))
